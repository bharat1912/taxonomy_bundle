...
server <- function(input, output){
  ...
  output$PCAplotBGC <- renderPlotly({
    ...
    plot <- ggplot(input_list[[1]], aes_string(x = 'X', y = 'Y', color = color_column, label= 'Sample'))  + xlab(paste0('PC1 - ', round(input_list[[2]][1],2), '%', sep='')) + 
      ylab(paste0('PC2 - ', round(input_list[[2]][2],2), '%', sep='')) + geom_point(aes_string(color = color_column)) + theme_bw() +  scale_color_manual(values = colorRampPalette(as.character(glasbey(32)))(nrow(input_list[[1]])))
    ggplotly(plot)
  ...
  output$dendograma <- renderPlot({
    ...
    unique_colors <- colorRampPalette(as.character(glasbey(32)))(length(unique_values))
    ...
    # Show tip labels at small size for readability
    plot(dend, type = "fan", no.margin = TRUE, edge.col = co,
         cex = 0.55, label.offset = 0.01)
  ...
  output$KNN <- renderPlotly({
    ...
    plot <- ggplot(knn, aes(x = X, y = Y, color = cluster, label = Sample)) + xlab("PC1")+
      ylab("PC2") + geom_point(aes(color = cluster)) + theme_bw()  +  scale_color_manual(values = colorRampPalette(as.character(glasbey(32)))(nrow(knn)))
    ggplotly(plot)
  })
  ...
  output$HDBSCAN <- renderPlotly({
    ...
    plot <- ggplot(hdbscan, aes(x = X, y = Y, color = cluster, label = Sample)) + xlab("PC1")+
      ylab("PC2") + geom_point(aes(color = cluster)) + theme_bw() +  scale_color_manual(values = colorRampPalette(as.character(glasbey(32)))(nrow(hdbscan)))
    ggplotly(plot)
  })
  ...
  output$PCAplotgenedistribution2 <- renderUI({
    ...
    plot <- ggplot(m, aes_string(x = 'X', y = 'Y', color = color_column, label= 'Sample'))  + xlab(paste0('PC1 - ', round(full_eig[1],2), '%', sep='')) + 
      ylab(paste0('PC2 - ', round(full_eig[2],2), '%', sep='')) + geom_point(aes_string(color = color_column)) + theme_bw() +  scale_color_manual(values = colorRampPalette(as.character(glasbey(32)))(nrow(copy_pcoa_data)))
    gg <- highlight(ggplotly(plot, height = 500), 'plotly_selected')
    p <- crosstalk::bscols(gg, DT::datatable(m), widths = 10,  device = 'lg')
