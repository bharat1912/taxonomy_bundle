...
rownames(full_matrix) <- make.unique(as.character(full_matrix$clusters)) 
...
rownames(gcf_pca_input) <- make.unique(as.character(gcf_pca_input$id))
...
GCF_eig <- GCF_pca_input[[2]]
  ...
  message("WARNING: BiG-SCAPE GCF matrix failed to load: ", e$message)
  GCF_pca <<- data.frame()
  GCF_eig <<- NULL
})
