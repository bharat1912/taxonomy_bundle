...
super_abs_pres_plot <- function(mtrx, groups, group1,group2, column){
  ...
  # Sanitise group names for use as column names (remove hyphens/spaces)
  group1_col <- gsub("-| ", "_", group1)
  group2_col <- gsub("-| ", "_", group2)
  
  df <- data.frame(id = mtrx[,1], group1= group1_means, group2 = group2_means)
  colnames(df)[2] <- group1_col
  colnames(df)[3] <- group2_col
  
  df2 <- df %>% group_by(across(all_of(c(group1_col,group2_col)))) %>% 
    summarise(total_count=n(),.groups = 'drop') %>%
    as.data.frame()
  
  plot <- ggplot(df2, aes_string(x= group1_col, y = group2_col, size = 'total_count')) + geom_point(alpha = 0.5,color = '#957DAD')  + scale_size(range = c(.1, 24), name="Number of gene clusters") + 
